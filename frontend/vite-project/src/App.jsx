...
const App = () => {
  ...
  const loadTemplate = () => {
    setCode(templates[language] || "// Start coding here...");
  };
  ...
  return (
    <div className="editor-container">
      <div className="sidebar">
        ...
        <button className="toggle-theme-button" onClick={toggleTheme}>{theme === "vs-dark" ? "Switch to Light Mode" : "Switch to Dark Mode"}</button>
        <button className="load-template-button" onClick={loadTemplate}>Load Template</button>
      </div>
      <div className="editor-wrapper">
        <Editor height="60%" language={language} value={code} onChange={handleCodeChange} theme={theme} options={{ minimap: { enabled: false }, fontSize: 14, quickSuggestions: true, parameterHints: { enabled: true } }} />
        <button className="run-btn" onClick={runCode}>Execute</button>
        <button className="save-btn" onClick={saveFile}>Save Code</button>
